...
// ── CONSTANTES OFICIALES DEL ÁLBUM ──────────────────────────────────────────
const TEAM_STICKERS_TOTAL    = 960;  // 48 equipos × 20 láminas
const SPECIAL_STICKERS_TOTAL = 34;   // Especiales(5) + Balón(4) + Historia(11) + Coca-Cola(14)
const GLOBAL_STICKERS_TOTAL  = 994;  // Total real del álbum
...
const ACHIEVEMENTS = [
  ...
  { id: "ach_993",  threshold: 993, icon: "👑", title: "ÁLBUM COMPLETO",   desc: "¡Leyenda absoluta del Mundial!" },
];
...
🤖 App by ByteBots · bytebots-figuritas-wc2026.vercel.app`
  ...
  // IDLE STATE — login required
  if (step === "idle") return (
    <div className="exchange-finder">
      {!user ? (
        <div className="ef-hero">
          <div className="ef-icon">📍</div>
          <div className="ef-title">Intercambios<br/>cerca de ti</div>
          <div className="ef-sub">Conecta con personas en tu ciudad que tienen las figuritas que te faltan — y viceversa</div>
          <div className="ef-stats">
            <div className="ef-stat"><span className="ef-num" style={{color:"var(--red)"}}>{myMissing.length}</span><span className="ef-lbl">Te faltan</span></div>
          ...
          <div style={{
            width:"100%",
            background:"linear-gradient(135deg,rgba(29,53,103,0.3),rgba(29,53,103,0.1))",
            border:"1px solid rgba(216,27,125,0.3)",
            borderRadius:"16px",
            padding:"24px 20px",
            textAlign:"center"
          }}>
            <div style={{fontSize:"13px",color:"rgba(242,244,247,0.6)",marginBottom:"16px",lineHeight:"1.6"}}>
          ...
          <div style={{fontSize:"11px",color:"rgba(242,244,247,0.25)",textAlign:"center",fontFamily:"var(--mono)"}}>
            Solo se usa para identificarte · No publicamos nada en tu cuenta
          </div>
        </div>
      ) : (
        <div className="ef-hero">
          ...
          <button className="ef-btn-primary" onClick={handleSetupStart} disabled={loading}>
            {loading ? "Detectando ubicación..." : "📍 Buscar en mi ciudad"}
          </button>
          <div style={{fontSize:"11px",color:"rgba(242,244,247,0.3)",fontFamily:"var(--mono)"}}>
        ...
        <div className="header-top">
          <div className="brand">
            ...
            <div>
              ...
              <div className="brand-sub">WC 2026 · 48 SELECCIONES · 994 LÁMINAS </div>
            </div>
          </div>
